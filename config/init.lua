...
do
  ...
  -- Diagnostic keymaps
  nmap("gE", function()
    return vim.o.spell and vim.fn.feedkeys("[s")
  ...
  vim.api.nvim_create_autocmd("PackChanged", {
    callback = function(ev)
      ...
      if name == "nvim-treesitter-textobjects" then
        -- Add queries to runtimepath without packadd
        -- (plugin init is incompatible with new nvim-treesitter)
              ...
              post = function()
                if vim.g.telescope_find_files_line then
                  vim.api.nvim_win_set_cursor(
                    0,
                    { vim.g.telescope_find_files_line, 0 }
                  )
                  vim.g.telescope_find_files_line = nil
                end
              end,
    ...
    vim.api.nvim_create_autocmd("LspAttach", {
      ...
      callback = function(event)
        ...
        vim.keymap.set(
          ...
          { buffer = buf, desc = "[G]oto References" }
        )
        vim.keymap.set(
          "n",
          "gI",
        ...
        vim.keymap.set(
          ...
          { buffer = buf, desc = "[A]ll workspace [S]ymbols" }
        )
      end,
    })
    ...
    local function lsp_progress()
      ...
      if status ~= "" then
        ...
        vim.g.lsp_progress_spinner = (vim.g.lsp_progress_spinner or 0)
            % #spinners
          + 1
        return spinners[vim.g.lsp_progress_spinner or 1]
          .. " "
